...
public class PeritiaEntityTextProvider implements IEntityComponentProvider
{
    @Override
    public void appendTooltip(ITooltip tooltip, EntityAccessor entityAccessor, IPluginConfig config)
    {
        if (!config.get(PeritiaJadePlugin.ENTITY_XP_SOURCES)) return;
        ...
        for (EntityXpSource xpSource : Peritia.xpSourceHandler().byType(PeritiaXpSourceTypes.ENTITY))
        {
            if (xpSource.inputs().test(entity))
            {
                if (xpSource.shouldShow(player, skillData))
                {
                    tooltip.addAll(PeritiaTexts.$entityInfo(xpSource, entity));
                }
    ...
    @Override
    public ResourceLocation getUid() { return PeritiaJadePlugin.ENTITY_XP_SOURCES; }
