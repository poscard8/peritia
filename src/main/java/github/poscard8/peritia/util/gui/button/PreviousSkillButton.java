...
@OnlyIn(Dist.CLIENT)
public class PreviousSkillButton extends CompactButton
{
    public PreviousSkillButton(Skill skill, int x, int y)
    {
        super(x, y, 9, 12, 194, 0, textureLocation(), press());
        setSkill(skill);
        setTextGetter(clientHandler ->
        {
            ...
            Skill[] skills = previousSkills();

            texts.add(PeritiaTexts.previousSkill());
            for (int i = 0; i < 3; i++)
            {
                Skill previous = skills[i];
                if (previous == null) break;

                texts.add(PeritiaTexts.skill(previous, colors[i]));
            }
            return texts;
        });
    }
    ...
    protected static OnPress press()
    {
        return button0 ->
        {
            if (button0 instanceof PreviousSkillButton button)
            {
                Skill previous = button.previousSkills()[0];
                if (previous != null)
                {
                    ...
                    minecraft.setScreen(new XpSourceScreen(previous));
    ...
    public Skill[] previousSkills()
    {
        ...
        {
            int index = i > originIndex ? i - originIndex - 1 : i + reordered.length - originIndex - 1;
            reordered[index] = allSkills[i];
        }

        for (int i = reordered.length - 2; i >= 0 && previousCount < 3; i--)
        {
            if (reordered[i] != null) skills[previousCount++] = reordered[i];
        }
        return skills;
    }
    ...
    {
        int distanceX = Math.abs(mouseX - (getX() + 9));
    ...
    {
        for (Skill skill : previousSkills())
        {
